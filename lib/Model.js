! function () {
  ...
  function Model (name, structure) {
    ...
    process.nextTick(function () {
      model.emit('debug',
        { 'new Model()': { name: name, structure: structure, tick: process._tickInfoBox[0] } });
    });
  ...
  Model.prototype.insert = function (row) {
    ...
    model.domain.run(function () {
      ...
      for ( field in this.structure ) {
        if ( model.structure[field].default ) {
          if ( typeof model.structure[field].default === 'function' ) {
            defaultRow[field] = model.structure[field].default(defaultRow[field]);
          }
          else {
            defaultRow[field] = model.structure[field].default;
          }
        }
      }

      for ( field in model.structure ) {
        if ( model.structure[field].required && ( ! ( field in defaultRow ) ) ) {
